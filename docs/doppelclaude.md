...
This verifies real Amp tools, independent conversation history, warm cache reads, automatic compaction and continuation after both a graceful dedicated-container restart and idle SIGKILL with explicit start. UI fork remains unverified because the test browser lacked Amp login; direct branch-import tests do not substitute for that UI check. Automatic restart-policy behavior, an in-flight crash, multi-minute silent-stream heartbeats and disconnect cancellation were not independently tested through this deployed route. The deployed service and Amp route remain in place; no debug logging was enabled.

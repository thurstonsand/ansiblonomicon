...
This verifies real Amp tools, independent conversation history, warm cache reads and continuation after both a graceful dedicated-container restart and idle SIGKILL with explicit start. It does not test automatic restart-policy behavior, an in-flight crash, UI fork/compaction, or independently measure multi-minute silent-stream heartbeats and disconnect cancellation. The deployed service and Amp route remain in place; no debug logging was enabled.

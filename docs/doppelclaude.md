...
## Stable personal deployment is ready
...
The existing personal Amp connection remains active, named `Doppelclaude`, with `anthropic-messages`, base URL `https://doppelclaude.thurstons.house/v1`, mapping `doppelclaude/*` and no warnings. No additional infrastructure or credential setup is required for personal use. Select a `doppelclaude/` model in the user's Amp mode: `doppelclaude/claude-opus-4-6` is the end-to-end tested choice. Current discovery advertises `claude-sonnet-5`, `claude-opus-5` and `claude-haiku-4-5`, selectable through the same prefix; these catalog entries are not an allowlist or a claim that each received the same commissioning coverage. Subscription availability still governs execution.

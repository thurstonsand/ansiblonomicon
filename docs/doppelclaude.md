...
After both conversations completed three turns and were idle, a separate bounded SIGKILL check killed only the dedicated container. Docker recorded exit 137, PID 0 and `OOMKilled=false` at `2026-09-19T07:30:14.489153917Z`. An explicit `docker start` produced start time `2026-09-19T07:30:14.806819728Z`, init PID 2781692 and daemon PID 2781753. The container became healthy and authenticated public `/v1/models` returned 200. A digest over image, container configuration, host configuration and mounts was identical before and after; no raw configuration values were emitted. Same-thread continuation after this abrupt stop remains to be checked by the application commissioning thread.

This verifies real Amp tools, independent conversation history, warm cache reads, continuation after a graceful dedicated-container restart and readiness after idle SIGKILL with explicit start. It does not test automatic restart-policy behavior, an in-flight crash, UI fork/compaction, or independently measure multi-minute silent-stream heartbeats and disconnect cancellation. The deployed service and Amp route remain in place; no debug logging was enabled.

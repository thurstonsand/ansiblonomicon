...
- The repository's existing `chezmoi/dot_config/amp/plugins/permission-gate/` is relevant: it protects selected git mutations and destructive removal, but when no plugin UI is available it rejects the protected action rather than paging a human. That is safe but means unattended repair must be able to stop and report.

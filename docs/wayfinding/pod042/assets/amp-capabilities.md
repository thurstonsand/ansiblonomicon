...
| Resident remote runner | `amp --no-tui [--runner-id pod042]` waits for remote thread-creation requests. Since 2026-09-17 a runner serves many directories: `--dir` repeated, `--discover-dirs` for every Git checkout up to two levels under the working directory, and `amp runner dirs add`, `remove`, and `list` on a live runner. `--remote-control-terminal` exposes a terminal to the web UI. | One systemd user service rooted at `~/code` covers every checkout, so a clone needs no unit change. The authority boundary is now the whole tree, not one repository. |
...
Amp did not document that a live ordinary `--no-tui` runner re-execs after an update, and its “services survive CLI updates” promise covered **Orb** services only. As of 2026-09-17 it does: `amp.runner.autoUpdate.enabled` defaults on, checks hourly, and restarts into the new release once no thread is running there, at most once per 12 hours. `amp.updates.mode: "disabled"` still turns it off. Systemd supervision remains the backstop, not the update mechanism.
...
- The repository retains a disabled `chezmoi/dot_config/private_amp/plugins/permission-gate/` implementation. If reinstated, it protects Git mutations, destructive removal, PostgreSQL mutations, and work web searches, but when no plugin UI is available it rejects protected actions rather than paging a human. Unattended repair must not assume those protections are active.

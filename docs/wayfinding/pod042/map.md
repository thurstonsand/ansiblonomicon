...
- [Secrets bootstrap for pod042](tickets/04-secrets-bootstrap.md) — reuse OpenClaw's SA (agent vault covers ~all refs); personal SSH key for git push; Amp API key from the agent vault; delivery via the new `op` launcher role per the secrets research; SA gets vault write access so the agent can self-provision credentials.

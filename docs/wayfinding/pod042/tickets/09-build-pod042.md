...
- 2026-07-25: The clean `.94` boot completed cloud-init without errors, grew the root filesystem to 78.5 GiB, detached its seed, cloned the repository through the forwarded 1Password Git key, and completed the first local converge. The first converge exposed and corrected pod042's missing Chezmoi service-account mode; the second converge and a post-reboot check both changed zero tasks. The on-VM Pi handoff remains.

...
Independent Hark delivery passed on physical pod042 at `4b605cd`, running from `/` with an empty environment. Native reconciliation reported zero changes; both pools remained ONLINE. The next patch adds the `maintenance` capability for GUID-guarded pool properties and SMART/ZED event monitoring, with no self-test schedules or scrub timers yet. Deployment and drive-capability probes remain pending review. Scheduled maintenance follows Healthchecks registration. The physical host uses America/Los_Angeles; monitoring schedules must match it. Its last ark scrub took 45 hours, so the scrub completion grace must allow that runtime.

Read-only Healthchecks inspection found only the default unused check and two down TrueNAS job checks, `truenas-pon-monitor` and `truenas-mam-update`. Neither new host nor scrub checks exist yet. The Hark integration exists. Reconcile the old job checks when their service replacements are addressed; do not treat them as pod042 coverage.

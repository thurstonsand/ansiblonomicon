...
The follow-up split generic Incus host, storage, and network policy from the Home Assistant workload capability. The running VM moved through Incus into the dedicated `black-box/incus` dataset without changing its UUID, MAC, resources, or UI-owned state. HAOS and `ha core check` passed after restart. Home Assistant owns its application configuration rather than receiving a parallel repo-managed YAML tree; Incus retains a working guest-agent control path for bounded diagnostics and future operations. The third-party HA-MCP integration provides authenticated native administration: Pi uses OAuth against its Nabu Casa endpoint, while Homepage, Amp, and Claude Code use the Home Assistant long-lived token. Claude Code moved to the token because its OAuth credential is keyed by server name and URL, so each rename or webhook change silently discarded it, and non-interactive sessions cannot log back in.

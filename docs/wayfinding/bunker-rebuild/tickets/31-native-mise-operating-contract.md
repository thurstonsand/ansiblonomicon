...
`mise pod042` auto-selects transport: it runs locally on hostname `pod042` and otherwise stages the same `bootstrap/targets/pod042` declaration over SSH. Units use concrete pod042 resources rather than speculative role defaults or exported environment interfaces. Capability config roots replace tags as the partial-reconciliation boundary; raw leaf tasks are not operator entrypoints. Full reconciliation performs declarative retirement, while a partial run may defer unrelated removals until the next full run.

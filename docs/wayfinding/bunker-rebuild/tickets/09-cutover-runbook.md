...
- **Phase 3 — bring-up**: `mise pod042:first-access` → native base and storage reconciliation → GUID-gated pool import and rename → verification battery. Ticket 42 records the completed landing zone; later capability tickets completed service restoration.
...
Cutover executed 2026-09-05 through the storage landing point. TrueNAS stopped services, took recursive `pre-debian-20260905T050835Z` snapshots, and cleanly exported both pools. Debian 13.6 replaced only Samsung SSD 870 EVO serial `S6P6NL0W307869J` after Intel VMD was disabled. The native landing target established key-only access and survived the destructive VM repeat. Both pools passed read-only GUID-gated inspection before explicit rename, mounted as `ark` and `black-box`, and returned healthy after reboot. Local apply and remote apply/check then reported every declared resource converged from the same clean revision. Subsequent capability tickets completed service restoration and final acceptance.

## Resolution

The Bunker rebuild is running on plain Debian 13 with native mise reconciliation. The pools were imported, renamed, rebuilt into the approved dataset layout, and stripped of the accepted legacy trees. Base host, networking, storage maintenance, SMB, Docker, retained Compose services, operator tooling, minimum Incus, and a fresh Home Assistant VM all passed their capability-level acceptance and converged from the repository. [Ticket 36](36-host-network-desired-state.md) records the permanently omitted disruption drills. [Backups](39-backup-and-alerting-desired-state.md) and [broader Incus agent workers](41-incus-agent-platform-fit.md) remain deferred tickets; camera setup and UPS integration wait without tickets until their hardware is ready.

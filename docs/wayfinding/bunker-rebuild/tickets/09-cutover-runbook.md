...
Route change 2026-09-04: the fresh Debian installation must never run the Ansible playbook. The existing playbook and gauntlet are now migration evidence only. [Native mise operating contract](31-native-mise-operating-contract.md) and its capability signoffs must produce and VM-prove the replacement before this ticket rewrites `bringup.sh`. `pod042-kvm` first becomes a permanent, independently powered console through [Bring up pod042-kvm](29-bring-up-pod042-kvm.md).

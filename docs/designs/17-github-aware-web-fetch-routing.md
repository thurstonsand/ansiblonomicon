...
Auth is a `GitHubAuth` dependency (`github-auth.ts`) injected into `createGitHubFetcher` and composed in `index.ts` alongside the fetcher array. Resolution order:
...
parallel-web-tools/
  ...
  github.ts       — GitHub fetcher: Octokit, URL parsing, resolvers, rendering
  github-auth.ts  — GitHubAuth token resolution, cached, injected via index.ts
...
The rule that matters: providers import from `contract.ts`/`shared.ts` only. No provider imports from a sibling provider or from the router, so `gitlab.ts` starts clean.
...
- [x] Phase 1: Delivery contract, router, and provider adaptation
  ...
  - Files: `contract.ts` (new), `router.ts`, `delivery.ts` (new), `shared.ts` (new), `github-auth.ts` (new), `parallel.ts`, `github.ts`, `fetch.ts`, `index.ts`, `search.ts` (imports only), package manifests.
...
- [ ] Phase 4 (deferred): Repository-scale fetching
  ...
  - Files: `github.ts` resolver addition.
...
- [ ] Phase 5 (deferred): Parallel session continuity via `session_id`
  ...
  - Files: `parallel.ts`, `search.ts`, possibly `contract.ts` if the session handle needs to cross the tool boundary.

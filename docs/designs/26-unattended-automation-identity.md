...
The source script runs while mise is still computing the project's PATH, so nothing it reaches may be a mise shim: a shim resolves the calling directory's environment before exec'ing its binary, which is this same script. Mise prepends the declared tools' real install directories, which covers `python3` and `fnox`, but `op` is a host tool and not declared. `fnox-host` therefore builds the resolver's PATH itself, dropping the shim directory and substituting the install a shim stood for where that was the only source of `op`.

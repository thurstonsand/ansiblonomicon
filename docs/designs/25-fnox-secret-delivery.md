...
A small repository launcher, `scripts/fnox-host`, exposes `profile`, `get NAME`, and `exec -- COMMAND ARG...`. Its dedicated Orb invocation selects the runner profile explicitly. It owns identity selection, absolute config location, authentication environment cleanup, and process execution. It does not parse SecretRefs, resolve values itself, cache values, or shadow `op`, `fnox`, or `chezmoi` on PATH.

...
- The local fetcher conforms fully to the design-17 contract: always-materialize, digest-only tool result, provider-authored excerpt.
...
- No `objective` steering in the local fetcher; that remains deferred to design 17's Phase 3 ephemeral-session direction.
...
The tool result shape is unchanged from design 17. A locally-fetched document is one of two kinds:
...
The chain is `[github, parallel, local]` on every machine, composed once in `index.ts`. Parallel's `canFetch` returns `false` when `PARALLEL_API_KEY` is unset and `true` otherwise — no conditional array composition, no environment branching in the router. This buys the case branching couldn't: when the key exists but Parallel is down or fails a specific URL, the failure becomes a per-URL attempt and the local fetcher recovers it.
...
`search_web` registration in `index.ts` remains gated on the key: without Parallel there is no search backend, and an unregistered tool is more honest than a permanently erroring one.
...
- **Next step:** Implement after the initial rollout proves the local path; needs the settings field and the declining wrapper in `index.ts` composition.
...
- **Status:** Rejected
- **Decision or open issue:** The hook system duplicates the router's per-source dispatch (its flagship GitHub-redirect hook is obsolete against the native GitHub fetcher). The LLM sub-agent and inline delivery contradict design 17's ratified decisions (single-completion summaries rejected; never inline). The cache served a re-prompt workflow that always-materialize replaces with stable artifacts.
...
- [x] Phase 1: Router throw-isolation and claim-time Parallel availability
  ...
  - Files: `router.ts`, `parallel.ts`, `index.ts`.
  - Work: Wrap `fetcher.fetch()` in the router; a throw becomes a `FailedAttempt` for every URL in that fetcher's claimed batch. Parallel's `canFetch` returns key presence. `index.ts` registers `search_web` only when the key is set.
...
- [x] Phase 3: Interactive login
  ...
  - Files: `fetch-worker.ts`, `worker-connection.ts`, `index.ts` (command registration).

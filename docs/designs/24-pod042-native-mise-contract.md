...
The stock Debian installer creates user `thurstonsand`, obtains Bunker DHCP, and installs OpenSSH. The first-access command:
...
stock Debian + DHCP + password SSH
  -> pod042:first-access hidden password prompt
...
The KVM already supplies reliable video, HID, and virtual media. Automating installer screens adds a fragile destructive program at the exact point where human confirmation has value. The operator owns installer confirmation; the first-access command begins only after Debian is reachable.
...
- **Discussion:** The cutover ticket records the target disk, and the first-access command manages the initial credential transition.
...
- [x] Phase 4: Cutover gate and runbook correction
  ...
  - Files: `docs/wayfinding/bunker-rebuild/tickets/09-cutover-runbook.md`, ticket 31, and the landing-zone implementation ticket.

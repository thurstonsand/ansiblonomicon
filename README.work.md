...
## One-time fnox activation review (design 25)
...
3. Route text templates through chezmoi `secret "NAME"`. Launch agents and local credential consumers through `scripts/fnox-host exec -- COMMAND`; their child environment supplies API tokens and MCP interpolation without exporting credentials into ordinary shells. Review all six corporate consumers: `ANTHROPIC_AUTH_TOKEN`, `BITBUCKET_TOKEN`, `GITLAB_ACCESS_TOKEN`, `LOGSCALE_VIPER_SPOG_TOKEN`, `N8N_API_KEY`, and `SOURCEGRAPH_TOKEN`.
4. Retire the old owned cache files and restart shells, tmux servers, editors, and agents that inherited them. Verify fresh host resolution, direct chezmoi, agent/MCP launch, and the work-local consumers without printing values. Record actual prompt behavior rather than assuming one biometric prompt.

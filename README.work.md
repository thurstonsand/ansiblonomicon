...
## One-time fnox activation review (designs 25, 26 and 27)
...
3. Route text templates through chezmoi `secret "NAME"`. Enroll the shared personal automation identity with `mise --no-env exec -- python3 scripts/automation_identity.py` before activating the repo's mise environment. Enrollment deliberately uses the personal `macos` fnox profile, even on the work host, to read the shared automation token; it does not enroll corporate credentials. Approve the configuration with `mise trust`. Agents launch normally; MCPs resolve their credentials at connection time. Any explicit `fnox-host exec` must name only its consumer's credentials with repeated `--secret NAME` options. Keep corporate and Private-vault access separate and attended. Review all six corporate consumers: `ANTHROPIC_AUTH_TOKEN`, `BITBUCKET_TOKEN`, `GITLAB_ACCESS_TOKEN`, `LOGSCALE_VIPER_SPOG_TOKEN`, `N8N_API_KEY`, and `SOURCEGRAPH_TOKEN`.

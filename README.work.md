...
## One-time fnox activation review (designs 25 and 26)

Work activation is pending. Before applying the consumer-switch revision on `ML-DFC6YK6VJQ`, perform this attended review on the work Mac:

1. Inspect the gitignored sources listed below, including shell templates, `local.toml` MCP entries, Claude overlays/hooks, local Ansible configuration/tasks, `agents/work/`, and `AGENTS.local.md`. Replace old `.env` readers, resolver calls, raw text-secret lookups, and shell-wide credential exports. Keep credentials out of review output and command arguments.
2. Obtain the complete corporate account UUID from `op account list` and set the nonsecret `FNOX_WORK_ACCOUNT` in the work-local shell configuration. The known prefix `EU7WV` is insufficient. Verify access to both the corporate and personal accounts; work deliberately requires both.
3. Route text templates through chezmoi `secret "NAME"`. Enroll the shared personal automation identity with `mise --no-env exec -- python3 scripts/automation_identity.py` before activating the repo's mise environment. Enrollment deliberately uses the personal `macos` fnox profile, even on the work host, to read the shared automation token; it does not enroll corporate credentials. Approve the configuration with `mise trust`. Agents launch normally, not through whole-host `fnox-host exec`; MCPs resolve their credentials at connection time. Keep corporate and Private-vault access separate and attended. Review all six corporate consumers: `ANTHROPIC_AUTH_TOKEN`, `BITBUCKET_TOKEN`, `GITLAB_ACCESS_TOKEN`, `LOGSCALE_VIPER_SPOG_TOKEN`, `N8N_API_KEY`, and `SOURCEGRAPH_TOKEN`.
4. Retire the old owned cache files and restart shells, tmux servers, editors, and agents that inherited them. Verify fresh host resolution, direct chezmoi, agent/MCP launch, and the work-local consumers without printing values. Confirm that the work `pi-web-tools@downgrade` package supports `webTools.parallel.apiKeyCommand` before removing its inherited Parallel key; the personal package implements on-demand resolution. Record actual prompt behavior rather than assuming one biometric prompt.
...
An Orb runner likewise needs its supplied identity verified against the complete shared set before activation. Its secret-free `.agents/resume` does not launch the application or modify the parent runner environment. The actual launch must explicitly use `scripts/fnox-host --orb exec -- COMMAND`, with `OP_SERVICE_ACCOUNT_TOKEN` supplied privately in the runner environment. Fnox removes provider authentication from the application child. No work or Orb live verification is implied by the committed configuration.
...
`~/.zshenv.local` and `~/.zshrc.local` are generated by the gitignored `chezmoi/private_dot_zshenv.local.tmpl` and `chezmoi/dot_zshrc.local.tmpl` on the work Mac. This keeps each file in both the chezmoi source and its deployed location without publishing work-specific configuration. Work-specific values live directly in those templates; text templates resolve credentials through chezmoi's native `secret` command, while explicit whole-host operations can use `scripts/fnox-host exec`. Do not restore global agent launch wrappers or provider-token exports. Change those sources rather than editing the rendered files.

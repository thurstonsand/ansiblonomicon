...
export async function runGhosttyPaste(
  exec: (command: string, args: string[]) => Promise<ExecResult>,
  outputDir: string,
): Promise<PasteResult> {
  const result = await exec("ghostty-nav", ["paste", "--output-dir", outputDir, "--json"]);
  if (result.code !== 0) {
    const detail =
  ...
  try {
    ...
    throw new Error(`ghostty-nav paste returned invalid JSON: ${String(error)}`);
  }

  return parseTypeBoxValue(PASTE_RESULT_SCHEMA, parsed, "Invalid ghostty-nav paste response");
}
...
export function fileNotification(result: PasteResult): string | undefined {
  switch (result.kind) {
    ...
    case "files": {
      ...
      if (mediaTypes.size === 1) parts.push([...mediaTypes][0] ?? "");

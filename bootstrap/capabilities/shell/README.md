...
Native mise owns `.zshenv`, `.zprofile`, `.zshrc`, the static Starship and direnv configuration, and work's `cleanh` helper. It preserves the existing `~/.zshenv.local` and `~/.zshrc.local` extension points and does not own those private work templates.

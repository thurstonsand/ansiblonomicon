...
The target's shared `vars.host_profile` selects `personal` or `work` behavior; this is host context available to every capability, not a Git-specific profile.

Run `mise git-client` from the repository for this capability alone, or add `--check` for a nonmutating preview. Regular laptop and pod042 reconciliation includes it. Check mode previews native dotfiles but does not execute hooks, so it does not validate raw corporate Git configuration.

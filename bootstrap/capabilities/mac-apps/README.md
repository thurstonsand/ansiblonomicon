...
Check mode retains Homebrew tap trust enforcement. Untrusted definitions are reported as errors rather than bypassed while checking drift. It uses `brew bundle check` to report missing or outdated declared entries, even when the daily interval would defer applying updates; undeclared transitive formulae are outside that check. Exit 0 means the Brewfile is satisfied; exit 1 reports drift. Operational errors also exit nonzero with diagnostics.

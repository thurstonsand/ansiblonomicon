...
`layout.toml` declares the six active filesystems. The native capability owns the `media` group and ACL package; its final hook converges ZFS properties through the GUID-guarded helper because mise has no dataset resource. `mise pod042 --check` also runs the helper's read-only check. Explicit mountpoints and local property values keep active children writable and mountable when their pool roots become unmounted, read-only legacy storage. `overlay=off` also prevents later mounts from silently covering files written into an unmounted destination.
...
There is no destruction phase here. Legacy retention is intentional until consumer migration and restore checks settle the remaining data. Cold zvols still reserve about 266 GB of otherwise free black-box capacity. Releasing that reservation would preserve their blocks but surrender guaranteed writable-restore capacity, so it requires separate approval. Same-pool snapshots and cloned files are rollback protection, not an independent backup. Pre-feature-upgrade snapshots do not reverse pool feature activation; their eventual removal requires a separate retention decision.

...
The existing OrbStack Debian VM proved repository installation before package installation, a real backports smartmontools install, an already-current upgrade, and detection/repair of induced repository-file mode drift. Physical pod042 then installed smartmontools 7.5-2~bpo13+1, read both NVMe self-test logs without the old namespace workaround, and converged with 45 unchanged resources. Subsequent commissioning completed short self-tests without errors on all eight drives.

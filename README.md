...
Shared Zsh startup files and static Starship configuration are also native mise resources; use `mise run shell` or `mise run shell --check`. Work rendering resolves its shell-wide Sourcegraph token and scoped sudo credential at apply time; check mode and personal/pod042 reconciliation perform no work-secret lookup. Project environments use mise; direnv remains installed for explicit use in external projects, without an automatic shell hook. See the [shell capability](bootstrap/capabilities/shell/README.md).

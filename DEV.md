...
- **Pi** at `chezmoi/private_dot_pi/agent/`: extensions under `extensions/`, permission rules under `permissions/`, external packages referenced from `settings.json.tmpl`. Bumping a workspace to pi 0.85.0 also needs a `@earendil-works/pi-server` devDependency: pi's barrel exports `main`, which imports its experimental server without declaring it, so importing pi at all throws. Drop the pin once upstream declares it.
